...
document.getElementById("myButton").onclick = function () {
  // get random index from cont
  let randomNumber = Math.floor(Math.random() * cont.length);

  // if new number = prev number make random number again
  ...
  document.getElementById(
    "myContent"
  ).innerHTML = `<p>"${cont[currentIndex].qoute}"</p>
        <p class="fw-bold">-- ${cont[currentIndex].aouther}</p>`;
};
